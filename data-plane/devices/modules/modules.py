...
parser.add_argument("--module-id", help="IoT Edge device module Id, i.e. $edgeHub. Exclude parameter to get all modules", required=False, default="")
...
moduleURI = 'https://%s/devices/%s/modules/%s?api-version=%s' % (resourceURI, deviceId, moduleId, apiVersion)
...
def get_modules():
    moduleTwinResponse = requests.get(moduleURI, 
        headers={
